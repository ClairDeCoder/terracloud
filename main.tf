...
resource "aws_instance" "terra-ec2" {
  ami           = "ami-007855ac798b5175e" # EC2 instance AMI ID
  instance_type = "t2.micro" # EC2 instance type
  private_ip    = "10.0.2.5" # Private IP of server
  ...
  security_groups = [aws_security_group.terra-sg.id] # requires bracket to indicate string
}

# security group for the EC2 instance
resource "aws_security_group" "terra-sg" {
  name_prefix = "terra-sg"
  vpc_id = aws_vpc.terra-vpc.id # reference terra-vpc

  # inbound rules for the security group
  ingress {
    from_port   = 22 # SSH access
    to_port     = 22 # SSH access
    protocol    = "tcp" # protocol
    cidr_blocks = ["192.168.2.0/24", "10.0.2.0/24"] # Allow networks
  }
...
# create private acl for logs-bucket to disable public access
...
resource "aws_s3_bucket_policy" "logs-bucket-policy" {
  bucket = aws_s3_bucket.logs-bucket.id # bucket name
...
resource "aws_route53_record" "cname6" {
  zone_id = aws_route53_zone.r53-zone.zone_id
  name    = "www.brandonsweat-terraform.net"
  type    = "CNAME"
...
resource "aws_cloudfront_origin_access_control" "OAC" {
  name                              = "brandonsweat-terraform-OAC"
